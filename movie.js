var utils = {
    ...
    createElementItem: function (movie) {
        ...
        $node.find('.cover img').attr('alt', movie.id);
...
$('.row img').click(function (event) {
    ...
    $('.moviedetail').show().siblings('.panel').hide().siblings('main').hide();
    var idUrl = $(this).attr('alt')
    // console.log(idUrl);
    var url = 'https://api.douban.com/v2/movie/subject/' + idUrl;
    $.ajax({
        url: url,
        type: 'GET',
        dataType: 'JSONP',
        success: function (data) {
            ...
            $('.detail h1').html(data.title + '(' + data.year + ')');
            $('.img img').attr('src', data.images.small);
            $('.font p:first').html("导演：" + data.directors.map(v => v.name).join('/'));
            $('.font p:eq(2)').html("演员：" + data.casts.map(v => v.name).join('/'));
            $('.font p:eq(3)').html("类型：" + data.genres.map(v => v).join('/'));
            $('.font p:eq(4)').html("制片国家/地区：" + data.countries.map(v => v).join('/'));
            $('.font p:last').html("又名：" + data.aka.map(v => v).join('/'));
            $('.abstract h3').html(data.title + "的剧情简介");
            $('.abstract p').html(data.summary);
            $('.movie h3').html(data.title + "的演职员");
            $('.director p').html(data.directors.map(v => v.name).join('/'));
            $('.director img').attr('src', data.directors[0].avatars.small);
            $('.cast').each(function (index, el) {
                $(this).find('img').attr('src', data.casts[index].avatars.small)
                $(this).find('p').text(data.casts[index].name)
            });
            $("#lunbo").hide();
            $("button").show();
        }
